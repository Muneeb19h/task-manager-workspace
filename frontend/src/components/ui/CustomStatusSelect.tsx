...
import type { FilterStatus } from '../../types/layout';
...
export const CustomStatusSelect: React.FC<CustomSelectProps> = ({ darkMode, statusFilter, setStatusFilter }) => {
  const [isOpen, setIsOpen] = useState(false);

  // Map out configs for cleaner rendering loops
  const options = [
    { value: 'All' as FilterStatus, label: 'Show All Tasks', icon: <FaGlobe className="text-indigo-400" /> },
    { value: 'Pending' as FilterStatus, label: 'Pending Status Only', icon: <FaHourglassHalf className="text-amber-500" /> },
    { value: 'In Progress' as FilterStatus, label: 'In Progress Only', icon: <FaBolt className="text-blue-400" /> },
    { value: 'Completed' as FilterStatus, label: 'Completed Only', icon: <FaCheckCircle className="text-emerald-400" /> },
  ];
  ...
  return (
    <div className="relative z-50 w-56">
      ...
      >
        ...
        <FaChevronDown className={`text-[10px] transition-transform duration-200 ${isOpen ? 'transform rotate-180' : ''}`} />
      </button>
      ...
      {isOpen && (
        <>
          ...
          <div className={`absolute left-0 right-0 mt-2 p-1.5 rounded-xl border shadow-xl z-50 animate-fadeIn ${
