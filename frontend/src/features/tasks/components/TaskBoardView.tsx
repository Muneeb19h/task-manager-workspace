...
export const TaskBoardView: React.FC<TaskBoardViewProps> = ({ darkMode, tasks, onEditSelect }) => {
  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6 items-start animate-fadeIn">
      {columns.map((col) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {/* ⏳ Renders the live dynamic target deadline string */}
                        {task.dueDate && (
                          <span className="text-[10px] font-mono text-slate-400/80 tracking-tight">
                            📅 {task.dueDate}
                          </span>
                        )}
                      </div>
